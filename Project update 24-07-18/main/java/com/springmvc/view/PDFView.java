...
public class PDFView extends AbstractITextPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		  ...
		  document.add(img2);
            ...
            Chunk title = new Chunk("Summery For ", subHeading);
			...
			Chunk name = new Chunk("NAME : " + user.getFirstname() + " " + user.getLastname(), subHeading);
            Paragraph p = new Paragraph(name);
            p.setLeading(10,0);
            document.add(p);
            document.add(Chunk.NEWLINE);
            
            Chunk account = new Chunk(String.valueOf("Account number : " +  user.getAccountnumber()),subHeading);
            account.setUnderline(1f, -3f);
            Paragraph p2 = new Paragraph(account);
            p2.setLeading(12,0);
            document.add(p2);
            document.add(Chunk.NEWLINE);
            
            Chunk address = new Chunk("Address : " + user.getAddress(), subHeading);
            Paragraph p3 = new Paragraph(address);
            p3.setLeading(12,0);
            document.add(p3);
            document.add(Chunk.NEWLINE);
            
            Chunk email = new Chunk("Email : " + user.getEmail(), subHeading);
            Paragraph p4 = new Paragraph(email);
            p4.setLeading(12,0);
            document.add(p4);
            document.add(Chunk.NEWLINE);
            
            Chunk contact = new Chunk("Contact :  " + user.getPhone(), subHeading);
            Paragraph p5 = new Paragraph(contact);
            p5.setLeading(12,0);
            document.add(p5);
            document.add(Chunk.NEWLINE);

...
public class LoginController {
  ...
  @ModelAttribute("login") Login login) {
    ...
    login.setPassword(getMD5.generate(login.getPassword()));
    ...
    if (user != null) {
        ...
        if(user.getDisabled().equals("true"))
        {
        	...
        	login.setDisplayedcaptcha(GenerateCaptcha());
        	login.setEnteredcaptcha(null);
        	mav3.addObject("login",login);
        	mav3.addObject("message", "You have been disabled. Contact Bank for furthur details.");
        	return mav3;
        }

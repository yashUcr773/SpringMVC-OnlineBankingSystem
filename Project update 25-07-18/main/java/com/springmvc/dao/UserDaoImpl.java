...
public class UserDaoImpl implements UserDao {
  ...
  public void register(User user) {
    String sql = "insert into users(username,password,ACCOUNTNUMBER,EMAIL,FIRSTNAME,LASTNAME,ADDRESS,PHONE,ACCOUNTTYPE,DOB,disabled) values(?,?,?,?,?,?,?,?,?,?,?)";
    jdbcTemplate.update(sql, new Object[] { user.getUsername(), user.getPassword(),user.getAccountnumber(),user.getEmail(),user.getFirstname(), user.getLastname(),user.getAddress(),user.getPhone(),"user",user.getDateofbirth(),"false"});
    }
  ...
  public Boolean checkIfExists(User user)
  {
	  ...
	  String sql2 = "select * from account where" + 
		  	   " accountnumber=" + user.getAccountnumber() +" and "+
		  	   " accounttype='" + user.getAccounttype() +"'";
	  
	  
	  List<Account> accounts = jdbcTemplate.query(sql2, new AccountMapper());
	  ...
	  String sql = "select * from users where" + 
    		  	   ...
    		  	   " accountnumber=" + user.getAccountnumber() +" and "+
    		  	   " phone='" + user.getPhone() +"' and "+
    		  	   " dob='" + user.getDateofbirth() +"' and "+
    		  	   " email='" + user.getEmail() +"' and "+
	  	...
	  	List<User> users = jdbcTemplate.query(sql, new UserMapper());
	  	 ...
	  	 if(users.size() > 0 && accounts.size()>0)
	  		 return true;
	  	else
	  		return false;
  }
